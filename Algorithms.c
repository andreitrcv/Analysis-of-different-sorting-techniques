#include "Algorithms.h"
    ...
    // - optimization of recursion call
    // - pseudo optimal pivot (median of three)
    // - base case for array of length < 93
        int k = 0;
        int q = 0;
        while(p < r){
            if(r - p + 1 > 93){
    ...
    void Merge(int array[], int p, int q, int r){
        ...
        // it would be better to create the array on the heap in order to avoid stack overflow
    ...
    void AdaptedInsertionSort(int array[], int p, int r){
     ...
     void HybridSort(int array[], int p, int r){
        int q;
        if  (r - p + 1 > 185){ // after 185 elements sort with Merge Sort
            q = (p+r)/2;
            MergeSort(array, p, q);
            MergeSort(array, q+1, r);
            Merge (array, p, q, r);
